 ...
 * Slug: harbour-bay-downtown/shop-categories
 * Categories: harbour-bay-downtown
 * Description: A segmented filter above a carousel of shop cards. Tabs are the Listing sub-categories that have shops (Listings → Type: Shops + a Category); clicking a tab shows that category's listings as a horizontal carousel. Falls back to static placeholders until shops are added.
 * Inserter: no
 * Viewport Width: 1440
 */

$tabs_data = hbd_get_listing_tabs( 'shops' );
...
$controls = '<div class="carousel__controls">'
	...
	. '</div></div>';
?>
<!-- wp:group {"tagName":"section","className":"shop-categories","layout":{"type":"default"}} -->
<section class="wp-block-group shop-categories">
	...
	<?php if ( ! empty( $tabs_data ) ) : ?>
		...
		<?php $first = true; foreach ( $tabs_data as $tab ) : ?>
		<div class="shop-categories__panel" data-panel="<?php echo esc_attr( $tab['term']->slug ); ?>" role="tabpanel"<?php echo $first ? '' : ' hidden'; ?>>
	...
	<?php else : ?>
		<?php
		// Placeholder until shops are tagged with categories.
		$ph_tabs  = array( 'Fashion & Lifestyle', 'Gifts & Souvenirs', 'Daily Essentials', 'Electronics & SIM' );
		$ph_cards = array(
			array( 'image' => 'card-shop.png',    'title' => 'Bayfront Boutique', 'date' => 'Daily' ),
			array( 'image' => 'promo-dining.png', 'title' => 'Island Souvenirs',  'date' => 'Open 10 AM' ),
			array( 'image' => 'card-stay.png',    'title' => 'Daily Mart',        'date' => 'Open 24h' ),
