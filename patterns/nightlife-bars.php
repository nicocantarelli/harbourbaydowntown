 ...
 * Slug: harbour-bay-downtown/nightlife-bars
 * Categories: harbour-bay-downtown
 * Description: Intro on the left + a carousel of venue cards, from Listings of type Nightlife in the "Bars & Lounges" category. Intro copy editable via Customizer → Nightlife Page → Nightlife — Bars & Lounges. Falls back to placeholders until venues are added.
 * Inserter: no
 * Viewport Width: 1440
 */

$pill        = get_theme_mod( 'hbd_nightlife_bars_pill', 'Bay Evenings' );
$heading_raw = get_theme_mod( 'hbd_nightlife_bars_heading', "Bars &\nlounges" );
$heading     = nl2br( esc_html( $heading_raw ) );
$body        = get_theme_mod( 'hbd_nightlife_bars_body', "Whether you're meeting friends, on a casual date, or extending dinner into drinks, there are nearby spots that suit different moods." );

$listings = hbd_get_listings( 'nightlife', -1, 'bars-lounges' );
if ( ! empty( $listings ) ) {
	$cards = array();
	...
	$cards = array(
		array( 'image' => $img . 'promo-dining.png', 'title' => 'Rooftop Cocktails', 'date' => 'Open nightly',  'link' => '#', 'is_external' => false ),
		array( 'image' => $img . 'promo-friday.png', 'title' => 'Live DJ Sets',      'date' => 'Fri–Sat, 9 PM', 'link' => '#', 'is_external' => false ),
		array( 'image' => $img . 'promo-surf.png',   'title' => 'Wine & Tapas',      'date' => 'Daily, 5 PM',   'link' => '#', 'is_external' => false ),
	);
}
?>
<!-- wp:group {"tagName":"section","className":"home-promotions nightlife-bars","layout":{"type":"default"}} -->
<section class="wp-block-group home-promotions nightlife-bars" data-carousel>
	...
	<div class="wp-block-group home-promotions__layout">
		...
		<div class="wp-block-group home-promotions__intro">
			<!-- wp:html -->
			<div class="nightlife-bars__tags">
				<span class="decor-ring" aria-hidden="true"></span>
				<span class="tag-chip"><?php echo esc_html( $pill ); ?></span>
			</div>
			<!-- /wp:html -->

			<!-- wp:group {"className":"nightlife-bars__copy","layout":{"type":"default"}} -->
			<div class="wp-block-group nightlife-bars__copy">
				<!-- wp:heading {"level":2,"className":"home-promotions__title"} -->
				<h2 class="wp-block-heading home-promotions__title"><?php echo $heading; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped — escaped via esc_html() above ?></h2>
				<!-- /wp:heading -->
