<?php
/**
 * Title: Live & Work — Enquire About Space
 * Slug: harbour-bay-downtown/livework-contact
 * Categories: harbour-bay-downtown
 * Description: A title + intro on the left and an enquiry form on the right. The form submits via AJAX (see inc/forms.php) and emails the Sales & Leasing inbox. Title/intro editable via Customizer → Live & Work Page → Live & Work — Enquire.
 * Inserter: no
 * Viewport Width: 1440
 */

$title = nl2br( esc_html( get_theme_mod( 'hbd_livework_contact_title', 'Enquire About Space' ) ) );
$body  = get_theme_mod( 'hbd_livework_contact_body', 'Interested in office space or accommodation at Harbour Bay Downtown? Share a few details below and our team will get back to you with suitable options.' );
?>
<!-- wp:group {"tagName":"section","className":"lw-contact","layout":{"type":"default"}} -->
<section class="wp-block-group lw-contact">
	<!-- wp:html -->
	...
	<form class="lw-contact__form" data-hbd-form="live-work" novalidate>
		...
		<div class="lw-contact__col">
			...
			<label class="lw-field">
				<span class="lw-field__label">I'm interested in</span>
				<select class="lw-field__input lw-field__input--select" name="enquiry_type">
