 ...
 * Slug: harbour-bay-downtown/home-promotions
 * Categories: harbour-bay-downtown
 * Description: Special promotions carousel with intro text, driven by the Events & Promotions content type (type: Promotion). Cards link to each promotion's detail page.
 * Block Types:
 * Viewport Width: 1440
 */

$promos = hbd_get_events( array( 'type' => 'promotion' ) );
if ( empty( $promos ) ) {
	$promos = hbd_placeholder_promotions(); // Show placeholders until the client adds Promotions.
}

$promo_title = nl2br( esc_html( get_theme_mod( 'hbd_promotions_title', "Special\npromotions" ) ) );
$promo_text  = get_theme_mod( 'hbd_promotions_text', "Stay updated with what's happening around Harbour Bay Downtown — from seasonal promotions to local events and new openings." );
?>
<!-- wp:group {"tagName":"section","className":"home-promotions","layout":{"type":"default"}} -->
<section class="wp-block-group home-promotions" data-carousel>
	...
	<div class="wp-block-group home-promotions__layout">
		...
		<div class="wp-block-group home-promotions__intro">
			...
			<h2 class="wp-block-heading home-promotions__title"><?php echo $promo_title; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped — escaped via esc_html() above ?></h2>
			<!-- /wp:heading -->

			<!-- wp:paragraph {"className":"home-promotions__lede"} -->
			<p class="home-promotions__lede"><?php echo esc_html( $promo_text ); ?></p>
			<!-- /wp:paragraph -->
		</div>
		<!-- /wp:group -->
